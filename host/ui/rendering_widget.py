...
class RenderingWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.waveform_plot.setTitle("Final Rendered Signal")
    ...
    def _show_time_signal(self, signal_data: np.ndarray):
        """Plot the exact single-cycle signal that will be streamed."""
    ...
    def _update_final_signal_plot(self):
        """Apply the selected loop processing and refresh the final-signal plot."""
        if not hasattr(self, "waveform_plot"):
            return
        source = self.playback_controller.playback_data
        if source is None or len(source) == 0:
            return
        if self.loop_check.isChecked():
            final_signal = make_boundary_crossfade_loop(
                source,
                self.sample_rate,
            ...
            self.waveform_plot.setTitle(
                f"Final Looped Signal · {self.boundary_window_combo.currentText()}"
            )
        else:
            final_signal = source
            self.waveform_plot.setTitle("Final Rendered Signal · no loop processing")
        self._show_time_signal(final_signal)
    ...
    def on_volume_changed(self):
        ...
        if self.playback_controller.playback_data is not None:
            ...
            self._update_final_signal_plot()
